...
# title           :load_db_w_screen.sh
# description     :This script will commit a sql script to a database specified
#                  in a configuration file defined in variable DATABASE_CFG_FILE.
#                  Alter file path to  configuration file to rungainst, developement,
#                  system test  acceptence or production
# author          :fsv
# date            :20120709
# version         :0.1    
#==============================================================================
...
# Function library for common functions
...
main()
{
    ...
    # Check that you're correct user
    correctUser
    ...
    if [[ $# != 1 ]]
    then
        echo 'Usage:' `basename ${0}` ' <screen sql script> '
        exit $ERROR
    fi
 
    # Save sql script name
    SQLFILE=$1
    ...
    # Make sure that the user wants to commit to correct database!
    yesOrNo $SQLFILE
