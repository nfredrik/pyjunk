...
class PriceRuleTest(unittest.TestCase):
	...
	def test_a_PriceRule_matches_when_it_mets_the_condition(self):
		rule = PriceRule("GOOG", lambda stock: stock.price > 10)
		self.assertTrue(rule.matches(self.exchange))

	def test_a_PriceRule_is_False_if_th_stock_not_in_exchange(self):
		rule = PriceRule("MSFT", lambda stock: stock.price > 10)
		self.assertFalse(rule.matches(self.exchange))

	def test_a_PriceRule_is_False_if_th_stock_hasnt_an_update_yet(self):
...
class AndRuleTest(unittest.TestCase):
	...
	def test_and_AndRule_matches_if_all_component_rules_are_true(self):
