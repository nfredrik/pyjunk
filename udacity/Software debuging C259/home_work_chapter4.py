...
def trace_fetch_state(frame, event, arg):
    ...
    #print 'iteration no:', current_iteration, the_line, the_iteration, frame.f_lineno, event
    # YOUR CODE HERE
    if the_line == frame.f_lineno and event == "line":
        current_iteration+=1
        the_iteration-=1
        #print 'current_iteration incremented!', the_line,  frame.f_lineno
        if the_iteration == 0:  
        #if current_iteration == the_iteration: 
            the_state = copy.deepcopy(frame.f_locals)
