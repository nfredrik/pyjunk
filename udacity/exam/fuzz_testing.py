...
# of the "file", and add the resulting byte array to a list. The return value of
# the fuzzit procedure should be a list of byte-modified strings.
...
def fuzzit(content):
    ...
    for item in buf:
        if item != 32:
            part.append(chr(item))
        else:    
            sum.append(''.join(part))
            part = []
